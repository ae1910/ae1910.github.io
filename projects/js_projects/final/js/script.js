...
let latestNewsAPI = `https://newsapi.org/v2/top-headlines?country=us&apiKey=${apiKey}`;
...
function showSlides(n) {
    ...
    for (i = 0; i < dots.length; i++) {
        dots[i].className = dots[i].className.replace("  check", "");
    }
    slides[slideIndex - 1].style.display = "block";  
    dots[slideIndex - 1].className += " check";
...
async function fetchLatestNews() {
    const requestURL = latestNewsAPI;
...
async function fetchCategoryNews() {
    for(let i = 0; i < categories.length; i++) {
        ...
        link =  categoryNewsAPI + category + "&apiKey=" + apiKey;
